...
import { ReactComponent as VerticalFileLine } from "../Resources/vertical_file_line.svg";
...
import { ReactComponent as FileTrigger } from "../Resources/file_trigger.svg";
import { ReactComponent as FileTriggerSelected } from "../Resources/file_trigger_selected.svg";
...
function WorkspaceFilesChooser({ workspaceName }) {
  ...
  const [showTipSecondPart, setShowTipSecondPart] = useState(true);
  ...
  const handleResize = () => {
    ...
    if (windowSize[1] <= 550) {
      // setShowTipSecondPart(false);
    } else {
      setShowTipSecondPart(true);
    }

    if (
      windowSize[0] <= screenSize[0] / 2 ||
      windowSize[1] < 800 ||
      (windowSize[0] < 1280 && windowSize[1] < 900)
    ) {
    ...
    return (
      <div className="right-box-info" id="right-box">
        <div className="info-box-content">
          <div id="textViewer" className="text-viewer">
            <h2 className="highweight-title">
              <span className="usual-text">{textName}</span>
            </h2>

            <p className="simple-paragraph">
              <span className="usual-text">{openedText}</span>
            </p>
          </div>
        </div>
      </div>
  ...
  const renderConnections = () => {
    if (showConnections) {
      ...
      return <div className="connections-div"></div>;
    }
  };

  const renderTipSecondPart = () => {
    if (showTipSecondPart) {
      return (
        <div>
          ...
          <div className="import-desc-box">
            <div className="zero-margin">
              <p className="simple-paragraph">
                <span className="usual-text">
                  Import needed notes from Obsidian, Notion, Evernote and
                  others! Click this magic button:
                </span>
              </p>
            </div>
          </div>
          ...
          <div className="import-button-div">
            <Link to={"/workspace_file_chooser"} className="simple-link">
              <button className="import-button">
                <p className="simple-paragraph">
                  <span className="usual-text">Import</span>
                </p>
              </button>
            </Link>
          </div>
        ...
        <div className="info-box">
          ...
          <div className="info-list-part">
            <div className="zero-margin">
              ...
              <p className="simple-paragraph">
                <span className="usual-text">
                  Bring them to life and create an explorable Space to share
                  with others!
                </span>
              </p>
            </div>
  ...
  const renderAvailableFileAction = (file) => {
    if (isCreatingSpace) {
      return (
        ...
        >
          {selectedIDForNewSpace.includes(file.id) ? (
            <FileTriggerSelected />
          ) : (
            <FileTrigger />
          )}
      ...
      return (
        <button className="cross-button" onClick={() => deleteFile(file.id)}>
          <Cross />
        </button>
      );
    }
        ...
        >
          ...
          <button
            className={`list-item-button-${
              data[index].title === textName ? "on" : "off"
            }`}
          >
            <div className="name-label-div">
              {/* <ListItemText primary={data[index].title}/> */}
              <p className="simple-paragraph">
                <span className="usual-text">{data[index].title}</span>
              </p>
            </div>
  ...
  const renderCreateSpaceOrCancle = () => {
    if (isCreatingSpace) {
      return (
        ...
        >
          <p className="simple-paragraph">
            <span className="usual-text">Cancel</span>
          </p>
        </button>
      );
    } else {
      return (
        <button className="new-space-button" onClick={newSpaceClicked}>
          <p className="simple-paragraph">
            <span className="usual-text">New Space</span>
          </p>
        </button>
      );
    }
  ...
  const renderUploadFilesOrContinue = () => {
    if (isCreatingSpace) {
      return (
        <button className="upload-files-button" onClick={() => {}}>
          <p className="simple-paragraph">
            <span className="usual-text">Continue</span>
          </p>
        </button>
      );
    } else {
      return (
        <button className="upload-files-button" onClick={selectFiles}>
          <Plus className="plus" />
          <p className="simple-paragraph">
            <span className="usual-text">Upload files</span>
          </p>
        </button>
      );
    }
      ...
      <div className="left-box" id="left-box">
        <div className="up-bar">
          ...
          <div className="searchbardiv">
            <Link className="simple-link" to={`/search_page/${workspaceName}`}>
              <button className="searchbar">
                <div>
                  <p className="simple-paragraph">
                    <span className="usual-text">Search</span>
                  </p>
                </div>
        ...
        <div className="bottom-div">
          <div className="new-space-div">{renderCreateSpaceOrCancle()}</div>
